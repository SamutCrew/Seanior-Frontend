...
import { TeacherCard, type Teacher } from "../Common/TeacherCard"
  ...
  return (
    ...
    >
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden">
        {/* Conditionally render fewer animations on mobile for better performance */}
        {typeof window !== "undefined" && window.innerWidth < 640 ? (
          // Simplified background for mobile
          <div className={`absolute inset-0 ${isDarkMode ? "bg-cyan-900/5" : "bg-blue-100/10"}`}></div>
        ) : (
      ...
      <div className="max-w-7xl mx-auto">
        <div className="text-center mb-16">
          ...
          <motion.h2
            ...
            className={`text-3xl sm:text-4xl md:text-5xl font-bold ${isDarkMode ? "text-white" : "text-gray-900"} mb-4 sm:mb-6`}
          >
          ...
          >
            <motion.div
              className={`w-12 sm:w-16 h-0.5 ${isDarkMode ? "bg-slate-700" : "bg-gray-300"} hidden sm:block`}
              initial={{ width: 0 }}
              animate={isInView ? { width: 64 } : { width: 0 }}
              transition={{ duration: 1, delay: 0.6 }}
            />
            <p
              className={`px-2 sm:px-6 ${isDarkMode ? "text-slate-300" : "text-gray-600"} max-w-2xl text-sm sm:text-base`}
            >
          ...
          {/* Teacher Cards */}
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6 md:gap-8 mt-8 sm:mt-12">
          ...
          <motion.div
            ...
            className="mt-10 sm:mt-16 px-4 sm:px-0"
          >
            <Link
              href="/teachers"
              className={`inline-flex w-full sm:w-auto items-center justify-center px-4 sm:px-6 py-3 border border-transparent text-sm sm:text-base font-medium rounded-md text-white ${
                isDarkMode
