...
export default function AvailableCourses({ availableCourses }: AvailableCoursesProps) {
  ...
  return (
    <div className={`${isDarkMode ? "bg-slate-800 border border-slate-700" : "bg-white"} rounded-lg shadow-sm`}>
      <div className={`p-4 ${isDarkMode ? "border-slate-700" : "border-b"} flex justify-between items-center`}>
        <div>
          <h2 className={`text-xl font-bold ${isDarkMode ? "text-white" : "text-slate-800"} flex items-center gap-2`}>
            <FaChalkboardTeacher className={`h-5 w-5 ${isDarkMode ? "text-cyan-400" : "text-cyan-600"}`} /> Available
            Courses
          </h2>
          <p className={`text-sm ${isDarkMode ? "text-gray-400" : "text-slate-500"} mt-1`}>
            Courses ready to be scheduled
          </p>
        </div>
        <button
          className={`${
        ...
        {availableCourses.length > 0 ? (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {availableCourses.map((course) => (
              ...
              >
                <h4 className={`font-bold text-lg ${isDarkMode ? "text-white" : "text-slate-800"}`}>{course.title}</h4>
                <div className="flex flex-wrap gap-2 mt-2 mb-4">
                  <span className={`text-sm px-2 py-1 rounded ${getLevelColor(course.level)}`}>{course.level}</span>
                  <span
                    className={`text-sm ${isDarkMode ? "bg-slate-700 text-gray-300" : "bg-slate-100 text-slate-800"} px-2 py-1 rounded flex items-center gap-1`}
                  >
                    <FaMapMarkerAlt className="h-3 w-3" /> {course.location}
                  </span>
                </div>
                <div className="flex justify-between items-center mt-4">
                  <span className={`text-sm ${isDarkMode ? "text-gray-400" : "text-slate-500"}`}>Not scheduled</span>
                  <button
                    className={`${
          ...
          <div className={`text-center py-12 ${isDarkMode ? "text-gray-400" : "text-slate-500"}`}>
            No available courses. Add a new course to get started.
          </div>
        )}
      </div>
    </div>
